...
class DeveloperProjectWebService
{
    ...
    public function getDeveloperProjectInfo($id)
    {
        ...
        if (isset($developer_project_entity)) {
            ...
            $data['publish_at'] = $developer_project_entity->created_at->format('Y年m月d日');
        }
        $length_name = 0;
        if(!empty($data['name'])){
    ...
    public function getDeveloperProjectByIds($ids)
    {
        ...
        $city_repository = new CityRepository();
        $developer_project_entities = $developer_project_repository->getDeveloperProjectByIds($ids);
        /** @var DeveloperProjectEntity $developer_project_entitie */
        foreach ($developer_project_entities as $developer_project_entitie) {
            $item = $developer_project_entitie->toArray();
            $item['developer_info'] = $developer_service->getDeveloperInfo($item['developer_id']);
            /** @var CityEntity $city_entity */
            $city_entity = $city_repository->fetch($item['city_id']);
            $item['has_decorate_name'] = $project_decorate_type_array[$item['has_decorate']] ?? '未知';
