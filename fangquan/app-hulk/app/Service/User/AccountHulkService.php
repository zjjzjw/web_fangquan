...
class AccountHulkService
{
    /**
     * @param WeixinLoginEntity $weixin_login_entity
     * @return array
     * @throws LoginException
     */
    public function login($weixin_login_entity)
    {
        $account = $weixin_login_entity->mobile;
        $ver_code = $weixin_login_entity->ver_code;
        $password = $weixin_login_entity->password;
        $ip = $weixin_login_entity->ip;
        //手机验证码是否正确
        $code = $this->validVerifyCode($account, $ver_code);
        if ($code != 200) {
     ...
     * @param $nickname
     */
    public function thirdPartyLogin($openid, $phone, $wx_avatar, $nickname)
    {
        $third_party_bind_repository = new ThirdPartyBindRepository();
        $third_party_bind_entity = $third_party_bind_repository->getThirdPartyByOpenId($openid);
        $fq_user_repository = new FqUserRepository();
        ...
        if (isset($fq_user_entity)) {
            ...
            $user_info_entity->user_id = $fq_user_entity->id;
            $user_info_entity->name = $nickname;
            $user_info_entity->company = '';
            $user_info_entity->phone = $phone;
            $user_info_entity->email = '';
            $user_info_entity->position = '';
            $user_info_entity->wx_avatar = $wx_avatar;
            $user_info_repository->save($user_info_entity);

            if (!isset($third_party_bind_entity)) {
                //第三方绑定表插入数据
                $third_party_bind_repository = new ThirdPartyBindRepository();
                $third_party_bind_entity = new ThirdPartyBindEntity();
                $third_party_bind_entity->open_id = $openid;
     ...
     * @return int
     */
    public function thirdPartyRegister($open_id, $nickname)
    {
        ...
        $fq_user_repository = new FqUserRepository();
        $fq_user_entity = $fq_user_repository->getUserByOpenId($open_id);
        if (!isset($fq_user_entity)) {
            $fq_user_entity = new FqUserEntity();
        }
        if (!empty($nickname)) {
     ...
     * @param $role_type
     */
    public function saveFqUserInfo($user_info, $role_type)
    {
        if ($user_info->user_id) {
            ...
            $fq_user_entity->role_type = $role_type;
            $fq_user_repository->save($fq_user_entity);
        }
    }
}
