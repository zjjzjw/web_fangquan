$(function () {
    ...
    var service = require('../../../service/developer/projectCategoryService');
    var index;
    ...
    $(document).on('click', '.add-btn', function () {
        ...
        var paramItem = '<li>' +
            ...
            '<input type="hidden" name="project[category-param' + index + '][param-name][]" value=" ' + $('.param-name').val() + '">' +
            '<input type="hidden" name="project[category-param' + index + '][param-key][]" value="' + generateUUID() + '">' +
            '</li>';
        if (paramName != '') {
    ...
    $(document).on('click', '.add-param-btn', function () {
        if ($('.category-name').val() == '') {
            ...
            var addParamHtml = '<p class="' + $('.category-name').val() + '"><span>'
                ...
                '<input type="hidden" name="project[category-param-name][]" value="' + $('.category-name').val() + '" >' +
                '<input type="hidden" name="project[category-param-key][]" value="' + generateUUID() + '">';
        ...
        if ($('.change-name').hasClass("add-name")) {
            ...
            if (addParam != '') {
                ...
                var addParamItem = '<li class="' + addAppendClass +
                    '"><span>' + addParam + '</span>' +
                    '<em class="delete">x</em><i class="iconfont edit-icon">&#xe626;</i>' +
                    '<input type="hidden" name="project[category-param' + index + '][param-name][]" value=" ' + addParam + '">' +
                    '<input type="hidden" name="project[category-param' + index + '][param-key][]" value="' + generateUUID() + '">' +
                    '</li>';
        ...
        if (nameVal == "") {
            ...
            service.store({
                ...
                sucFn: function (data, status, xhr) {
                    ...
                    function skipUpdate() {
                        $successPop.closePop();
                        window.location.href = '/developer/project-category/index';
                    }
                },
