exports.autoGenerate = (function (dir) {
    var fs = require('fs'),
        ...
        outputFile = path.normalize(dirname + '/../build.json');
    ...
    function getManifest(dir) {
        ...
        for (var i = 0, len = files.length; i < len; i++) {
            ...
            if (-1 === excludes.indexOf(file) && /\.js/.test(file)) {
                filename = file.substr(0, file.length - 3);
                dependencies.push(_jsonManifest(filename));
            }
        }
        return dependencies;
    }
    ...
    function _jsonManifest(file) {
        var temp = {};
        temp['pages.' + file.replace('/', '.')] = 'pages/' + file + '.' + uuid.v1().replace(/-/gi, '');
        return temp;
    }
